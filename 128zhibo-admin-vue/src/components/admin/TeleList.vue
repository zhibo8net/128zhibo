<template>
  <div>
      <h2>电视管理</h2>
      <div><button type="button" class="btn btn-info pull-right" v-on:click="create()">新建电视</button></div>
      <table class="table">
        <thead>
          <tr>
            <th>名称</th>
...
export default {
  ...
  methods: {
    ...
    edit: function (tele) {
      g.href('/teleForm/' + tele.id)
    },
    remove: function (tele) {
      var that = this
      Vue.http.delete(g.t('/api/admin/teles/' + tele.id)).then(function (response) {
        var teles = that.$data.page.content
        teles.splice(teles.indexOf(tele), 1)
      }, function (response) {
        g.toLogin()
      })
    },
    goto: function (page) {
      var that = this
      Vue.http.get(g.t('/api/admin/teles/page?page=' + page)).then(function (response) {
