<template>
  <div>
      <h2>组图管理</h2>
      <div><button type="button" class="btn btn-info pull-right" v-on:click="create()">新建组图</button></div>
      <table class="table">
        <thead>
          <tr>
            <th>标题</th>
...
export default {
  ...
  methods: {
    create: function () {
      location.href = '#/imageForm/'
    },
    edit: function (image) {
      location.href = '#/imageForm/' + image.id
    },
    remove: function (image) {
      var that = this
      Vue.http.delete(g.t('/api/admin/imageBags/' + image.id)).then(function (response) {
    ...
    goto: function (page) {
      var that = this
      Vue.http.get(g.t('/api/admin/imageBags/page?page=' + page)).then(function (response) {
